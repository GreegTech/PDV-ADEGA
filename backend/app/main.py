...
def products(include_inactive:bool=False,db:Session=Depends(get_db),user:User=Depends(current_user)):
...
def create_product(data:ProductCreate,db:Session=Depends(get_db),user:User=Depends(current_user)):
 ...
 p=Product(**data.model_dump(exclude={"barcode"}),barcode=barcode,active=True)
 try:
  db.add(p);db.flush()
...
def reactivate_product(product_id:int,db:Session=Depends(get_db),user:User=Depends(current_user)):
 p=db.get(Product,product_id)
 if not p:raise HTTPException(404,"Produto não encontrado")
...
def dashboard(db:Session=Depends(get_db),user:User=Depends(current_user)):
 return {"products":db.scalar(select(func.count(Product.id)).where(Product.active==True)) or 0,"stock_units":int(db.scalar(select(func.coalesce(func.sum(Product.stock),0))) or 0),"low_stock":db.scalar(select(func.count(Product.id)).where(Product.active==True,Product.stock<=Product.min_stock)) or 0,"sales_total":float(db.scalar(select(func.coalesce(func.sum(Sale.total),0))) or 0),"discount_total":float(db.scalar(select(func.coalesce(func.sum(Sale.discount_total),0))) or 0),"cmv_total":float(db.scalar(select(func.coalesce(func.sum(Sale.cmv_total),0))) or 0),"gross_margin":float(db.scalar(select(func.coalesce(func.sum(Sale.gross_margin),0))) or 0)}
