...
def startup():
    ...
    try:
        ...
        if not db.scalar(select(User).where(User.username==username)):
            db.add(User(username=username,password_hash=hash_password(os.environ["ADMIN_PASSWORD"]),role="admin")); db.commit()
...
def create_purchase(data:PurchaseCreate,db:Session=Depends(get_db),user:User=Depends(current_user)):
    ...
    try:
        for line in data.items:
            ...
            qty=line.quantity; incoming=money(line.unit_cost); old_stock=product.stock; old_avg=money(product.cost); new_stock=old_stock+qty
            new_avg=money(((old_avg*old_stock)+(incoming*qty))/new_stock); line_total=money(incoming*qty); total+=line_total
